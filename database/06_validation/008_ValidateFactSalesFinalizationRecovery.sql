...
Script   : 008_ValidateFactSalesFinalizationRecovery.sql
Author   : Diego Suárez
...
IF OBJECT_ID
(
    N'audit.CK_ETLWatermark_FactSales_FinalizationFailure',
    N'C'
) IS NOT NULL
BEGIN
...
BEGIN TRY
    ...
    ADD CONSTRAINT CK_ETLWatermark_FactSales_FinalizationFailure
        CHECK
        (
            ProcessName <> N'etl.LoadFactSalesIncremental.Header'
            OR [Status] <> 'Ready'
        );
    ...
    IF ISNULL(@CaughtErrorNumber, -1) <> 547
       OR CHARINDEX
          (
              N'CK_ETLWatermark_FactSales_FinalizationFailure',
    ...
    BEGIN
        THROW 51311, 'Committed fact changes were not preserved after finalization failure.', 1;
    END;
    ...
    ALTER TABLE audit.ETLWatermark
    DROP CONSTRAINT CK_ETLWatermark_FactSales_FinalizationFailure;
...
PRINT 'PASS: Fact commit, finalization failure, idempotent retry, and cleanup.';
GO
